...
def load_plate_image(plate_number):
    """Try to load Ishihara plate from various GitHub sources"""
    image_sources = [
        # Primary source - well maintained repo
        f"https://raw.githubusercontent.com/ishihara-test/plates/main/plate{plate_number}.png",
        # Backup source
        f"https://raw.githubusercontent.com/color-vision/ishihara/master/plates/{plate_number}.jpg",
        # Another backup
        f"https://raw.githubusercontent.com/vision-test/ishihara-plates/main/plate{plate_number}.png"
    ]
...
def ishihara_test():
    ...
    plate_image = load_plate_image(plate_number)
    
    if plate_image:
        st.image(plate_image, width=400, caption=f"Plate {plate_number}")
        st.success("✅ Real Ishihara plate loaded")
    else:
        # Fallback - create a colored circle placeholder
        st.warning("⚠️ Using color pattern simulation (real plates not available)")
        st.info("🔴 🟢 Color Vision Test Pattern 🔵 🟡")
        st.write("For accurate testing, consult standard Ishihara plate books")
    ...
    with st.expander("📋 Test Instructions"):
        ...
        - Note: Online images may vary from standard plates
        """)
