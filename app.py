...
def ishihara_test():
    ...
    with col2:
        ...
        if os.path.exists(image_path):
            # Koristi st.image s fiksnom širinom
            st.image(image_path, width=300, use_column_width=False)
            
            # Dodaj tekst ispod slike koji objašnjava što se vidi
            st.markdown("---")
            if plate_data["normal"] != "":
                st.info(f"**Normal color vision sees:** {plate_data['normal']}")
            else:
                st.info("**Normal color vision sees:** Nothing")
            
            if plate_data["deutan"] != "":
                st.info(f"**Red-green deficiency sees:** {plate_data['deutan']}")
            else:
                st.info("**Red-green deficiency sees:** Nothing or wrong number")
    ...
    with col2:
        user_answer = st.text_input(
            ...
            help="Enter the number you see, or leave blank if you don't see any number"
        )
    ...
    with col3:
        if st.button("🔄 Other Tests", use_container_width=True):
            ...
            st.rerun()
    with col4:
        if current_index < TOTAL_PLATES - 1 and st.button("Next ➡️", use_container_width=True, type="primary"):
            st.session_state.current_plate += 1
            st.rerun()
    with col5:
