...
# Test data - CORRECTED Ishihara interpretations
ISHIHARA_DATA = {
    1: {"normal": "12", "deutan": "12", "protan": "12"},
    2: {"normal": "8", "deutan": "3", "protan": "8"},
    3: {"normal": "6", "deutan": "5", "protan": "6"},
    4: {"normal": "29", "deutan": "70", "protan": "29"},
    5: {"normal": "57", "deutan": "35", "protan": "57"},
    6: {"normal": "5", "deutan": "2", "protan": "5"},
    7: {"normal": "3", "deutan": "5", "protan": "3"},
    8: {"normal": "15", "deutan": "17", "protan": "15"},
    9: {"normal": "74", "deutan": "21", "protan": "74"},
    ...
    18: {"normal": "", "deutan": "5", "protan": ""},
    19: {"normal": "", "deutan": "2", "protan": ""},
    20: {"normal": "", "deutan": "45", "protan": ""},
    21: {"normal": "", "deutan": "73", "protan": ""},
...
def generate_certificate(user_data, test_results):
    ...
    pdf.cell(0, 8, f'ID: {user_data["id"]}', 0, 1)
    pdf.cell(0, 8, f'Position: {user_data["position"]}', 0, 1)
    pdf.cell(0, 8, f'Date of Assessment: {user_data["date"]}', 0, 1)
    pdf.ln(10)
    ...
    for test in test_results:
        pdf.cell(60, 10, test['test'], 1, 0, 'L')
        pdf.cell(40, 10, test['score'], 1, 0, 'C')
        pdf.cell(40, 10, test['accuracy'], 1, 0, 'C')
        status_color = (0, 128, 0) if test['status'] == 'PASS' else (255, 0, 0)
        pdf.set_text_color(*status_color)
        pdf.cell(50, 10, test['status'], 1, 1, 'C')
    ...
    if passed_tests == total_tests:
        ...
        pdf.set_font('Arial', '', 12)
        pdf.multi_cell(0, 8, 'The candidate has demonstrated satisfactory color vision capabilities for maritime duties. Performance meets IMO standards for navigation and lookout responsibilities.')
    else:
        pdf.set_text_color(255, 0, 0)
        pdf.set_font('Arial', 'B', 16)
        pdf.cell(0, 12, '❌ OVERALL RESULT: FAIL', 0, 1, 'L')
        pdf.set_font('Arial', '', 12)
        pdf.multi_cell(0, 8, 'The candidate has not met the required standards for color vision in maritime operations. Further professional assessment is recommended.')
    ...
    pdf.multi_cell(0, 8, 'This certificate is issued based on computerized color vision assessment. For official medical certification, consult a qualified maritime medical examiner.')
...
def home_page():
    ...
    col1, col2, col3, col4 = st.columns(4)  # PROMJENJENO: 4 stupca umjesto 3
    ...
    with col4:
        ...
        <div class="test-card">
            ...
            <p>Radar-specific color discrimination</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    if any(key in st.session_state for key in ['lantern_answers', 'user_answers', 'ecdis_scores', 'radar_scores']):
        ...
        col1, col2, col3, col4 = st.columns(4)  # PROMJENJENO: 4 stupca
...
def ishihara_test():
    ...
    with col2:
        ...
        if os.path.exists(image_path):
            st.image(image_path, width=440, use_column_width=True)
        else:
...
def calculate_ishihara_score():
    ...
    for plate_num in range(1, 25):
        ...
        # For plates 1-17: normal vision sees normal number
        if plate_num <= 17:
            if user_answer == plate_data["normal"]:
                score += 1
        # For plates 18-21: colorblind sees the number (normal sees nothing)
        elif plate_num <= 21:
            if user_answer == "" and plate_data["normal"] == "":  # Normal sees nothing
                score += 1
            elif user_answer != "" and plate_data["normal"] == "":  # Colorblind sees number
                score += 0  # This indicates color deficiency
        # For plates 22-24: both see different numbers
        else:
            if user_answer == plate_data["normal"]:  # Normal sees normal number
                score += 1
...
def critical_color_pairs_test():
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("SAME COLORS", use_container_width=True):
            st.session_state.radar_pair_answers.append(is_same == True)
            st.session_state.radar_pair_index += 1
            st.rerun()
    with col2:
        if st.button("DIFFERENT COLORS", use_container_width=True):
...
def intensity_ordering_test():
    ...
    # Check order button
    if st.button("Check Order", type="primary"):
...
def night_mode_test():
    ...
    # Simulate night radar with random targets
    if 'radar_night_answer' not in st.session_state:
        st.session_state.radar_night_targets = random.randint(1, 5)
        st.session_state.radar_night_start = time.time()
    
    num_targets = st.session_state.radar_night_targets
    ...
    # User input
    user_guess = st.number_input("How many targets do you see?", min_value=0, max_value=10, value=0)
...
def show_results():
    ...
    if st.session_state.get('user_answers'):
        ...
        if ishihara_score >= 17:  # Normal color vision typically scores 17+
